...
load("../Tidy-GHG-Inventories/data/data_crts_v1.3.RData") #Also available here: https://github.com/lambwf/Tidy-GHG-Inventories/tree/main/data
...
## Make a strict inclusion criteria where all 6 indicators need to be negative
...
inclusions <- inclusions %>%
  left_join(.,inclusions %>% filter(strict_include>=5 & var=="value_inLUC_10yr" & include==0 | 
                                      strict_include>=5 & var=="value_co2cons_10yr" & include==0) %>% 
...
for (i in 1:length(countries)) {
  ...
  p1 <- plot_data %>% 
    ...
    labs(title=paste0(countries[i]),
         ...
         caption="⌂ Data: UNFCCC, Tidy GHG inventories, GCB 2024   ⌂ CC-BY William F. Lamb  \n⌂ Note: GWP100 AR5; net land use change (LULUCF) emissions based on inventories")
...
writeData(wb, sheet = "data",data_totals %>%
            ...
            mutate(units="MtCO2e (GWP100 AR5)"),colNames = T, rowNames = F)
...
data_sectors <- data_crts %>% 
  left_join(.,read.xlsx("../Tidy-GHG-Inventories/data/cc_inventory_sectors.xlsx",sheet=2) %>%
              select(sector_code,sector_tidy,sector_tidy_order),by=join_by(sector_code))
...
data_sectors <- data_sectors %>% 
  ...
  group_by(country,iso) %>% 
  filter(year %in% c(first(peak_year),max(year)))
